...
"""
Here is a script that scrapes the Mutopia Project at mutopiaproject.org for midi music. The midi files will be saved
...
class SheetMusicScraper:
    ...
    def scrape(self, path):
        """
        Scrape mutopiaproject.org for all of its music in midi format and save it to the path provided
        :param path: the path at which to save the midi files
        """
        ...
        for i in range(0, 2111, 10):
            ...
            while True:
                try:
                    ex = re.search(r'(<td><a href).*\.mid file', box)
                    midi_link = ex.group(0)[13:-11]
                    # print(midi_link)
                    self.midis.append(midi_link)
        ...
        # Now that we have the list of links that download the midis, we must request them and save them
        num = 0
        if (len(self.midis) == len(self.names)) and (len(self.midis) == len(self.composers)):
            print("Length check: Passed :)")
            num = len(self.midis)
        else:
            print("Length check: Failed :(")
            print("MIDIs Length:", len(self.midis))
